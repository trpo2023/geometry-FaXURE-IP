...
#define CTEST_IMPL_STRUCT(sname, tname, tskip, tdata, tsetup, tteardown)                           \
    static struct ctest CTEST_IMPL_TNAME(sname, tname) CTEST_IMPL_SECTION = {                      \
        #sname, #tname, { (ctest_nullary_run_func)CTEST_IMPL_FNAME(sname, tname) }, tdata,         \
        (ctest_setup_func*)tsetup, (ctest_teardown_func*)tteardown, tskip, CTEST_IMPL_MAGIC,       \
    }
...
#define CTEST_IMPL_CTEST2(sname, tname, tskip)                                                     \
    ...
    CTEST_IMPL_STRUCT(sname, tname, tskip, &CTEST_IMPL_DATA_TNAME(sname, tname),                   \
        &CTEST_IMPL_SETUP_TPNAME(sname, tname), &CTEST_IMPL_TEARDOWN_TPNAME(sname, tname));        \
    static void CTEST_IMPL_FNAME(sname, tname)(struct CTEST_IMPL_DATA_SNAME(sname) * data)
...
void assert_compare(const char* cmp, intmax_t exp, intmax_t real, const char* caller, int line)
{
    ...
    {
        CTEST_ERR(
            "%s:%d  assertion failed, %" PRIdMAX " %s %" PRIdMAX "", caller, line, exp, cmp, real);
    }
}
    ...
    const char* cmp, double exp, double real, double tol, const char* caller, int line)
{
    ...
    if (!get_compare_result(cmp, c3, eq))
    {
        ...
        CTEST_ERR("%s:%d  assertion failed, %.8g %s %.8g (diff %.4g, %s %.4g)", caller, line, exp,
            cmp, real, diff, tolstr, tol);
    }
}
...
// if we got here it's ok
    ...
    snprintf(results, sizeof(results),
        "RESULTS: %d tests (%d ok, %d failed, %d skipped) ran in %.1f ms", total, num_ok, num_fail,
        num_skip, (double)(t2 - t1) * 1000.0 / CLOCKS_PER_SEC);
